...
export class AuthService implements IAuthService {
  constructor(
    ...
    @inject(TYPES.LoginActivityService)
    private _loginActivityService: ILoginActivityService,
  ) { }
  ...
  async login(
    ...
    if (clientInfo) {
      await this._loginActivityService.recordLogin(
        user._id as unknown as string,
        clientInfo.ip,
      ...
      this._emailService.sendLoginAlertEmail(
        user.email,
        user.profile.firstName,
        "Unknown Device",
      );
