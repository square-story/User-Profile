...
export class AdminController {
  ...
  updateUser = async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const updatedUser = await this._adminService.updateUser(
  ...
  deactivateUser = async (
    ...
    try {
      await this._adminService.deactivateUser(req.user?.userId as string, req.params.id);
      res
  ...
  reactivateUser = async (
    ...
    try {
      await this._adminService.reactivateUser(req.user?.userId as string, req.params.id);
      res
        .status(StatusCode.Success)
  ...
  bulkDeactivate = async (
    ...
    try {
      ...
      await this._adminService.bulkDeactivateUsers(req.user?.userId as string, userIds);
      res
        .status(StatusCode.Success)
  ...
  bulkReactivate = async (
    ...
    try {
      const { userIds } = req.body;
      await this._adminService.bulkReactivateUsers(req.user?.userId as string, userIds);
      res
        .status(StatusCode.Success)
        .json({ success: true, message: "Users reactivated successfully" });
    } catch (error: unknown) {
      next(error);
    }
  };
  ...
  getAuditLogs = async (req: Request, res: Response, next: NextFunction) => {
    try {
      const result = await this._adminService.getAuditLogs(
  ...
  getUserLoginHistory = async (
    ...
    try {
      const history = await this._adminService.getUserLoginHistory(
        req.params.id,
      );
